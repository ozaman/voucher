var app = angular.module('myApp', ['ngCookies','ngRoute','angular.filter','base64','ui.bootstrap','FBAngular','geolocation','ui-notification','ngAnimate','ngMessages','ngImgCrop']);
    app.controller('myCtrl', function($scope,$document,$sce,$base64,$location,$http,$window,$filter,$templateCache,$interval,$timeout,$cookies,$cookieStore,Fullscreen,geolocation,$anchorScroll,Notification){
       ...
       // Notification.warning({message: 'notification message from driver', delay: null});
        ...
        $scopeset_color = function (payment) {
        if (payment == 'In') {
            return {
                color: "red"
            }
            ...
            if (i.TB_order_type == 'transfer') {
                if (i.area == 'Rental') {
                   ...
                   $http({
                    method : 'POST',
                          ...
                          data: $.param({id: i.TB_order_program ,sv: $scope.dataSV}),
                          headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                        }).success(function(res){                  
                          console.log(res)
                          console.log()
                ...
                angular.element(document.querySelector('#popnavi')).css("margin","20px 3%");
                    ...
                    geolocation.getLocation().then(function(data){
                                    ...
                                    $scope.latituderel = $scope.coords.lat;
                                    $scope.longituderel = $scope.coords.long;
                                    $scope.iframe = {navilink:"https://www.google.com/maps/embed/v1/directions?key=AIzaSyDJa08ZMaSnJP5A6EsL9wxqdDderh7zU90&destination="+i.TB_transferplace_topic+"&origin="+$scope.latituderel+","+$scope.longituderel+"&avoid=tolls|highways&zoom=12?hl="+$scope.checklang};
            ...
            if (i.TB_order_type != 'transfer') {
                ...
                $http({
                    method : 'POST',
                          ...
                          data: $.param({id: i.TB_order_order ,sv: $scope.dataSV}),
                          headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                        }).success(function(res){                  
                          console.log(res)
                          //$scope.nontifigcomment = res;
 ...
 //  });
            ...
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                ...
                for (var i = 0; i < $scope.datebooks2.length; i++) {
                    var s = 0;
                        angular.forEach($scope.VocherID, function (y) {
                            var ondate = y.TB_order_outdate.split("-"); 
                           // console.log(ondate)
               ...
               angular.element(document.querySelector('#load')).css("display","none");
                ...
                console.log($scope.numVocherpass5)
                 if($scope.flagcheck == 1){
                    $scope.Typetext = "Unused"+' '+'('+$scope.Voucherwait.length+')';
                }
                if($scope.flagcheck == 2){
                    $scope.Typetext = "รอใช้"+' '+'('+$scope.Voucherwait.length+')';
                }
                if($scope.flagcheck == 3){
                    $scope.Typetext = "等待使用"+' '+'('+$scope.Voucherwait.length+')';
                }
        ...
        $scope.selectType = function(x){
            ...
            if (x==2) {
                if($scope.flagcheck == 1){
                    $scope.Typetext = "Used" +' '+'('+$scope.Voucherpass.length+')';
                }
                if($scope.flagcheck == 2){
                    $scope.Typetext = "ใช้แล้ว" +' '+'('+$scope.Voucherpass.length+')';
                }
                if($scope.flagcheck == 3){
                    $scope.Typetext = "已经使用" +' '+'('+$scope.Voucherpass.length+')';
                }
            ...
            if (x==4) {
                if($scope.flagcheck == 1){
                    $scope.Typetext = "Canceled"+' '+'('+$scope.Vouchercancel.length+')';
                }
                if($scope.flagcheck == 2){
                    $scope.Typetext = "ที่ถูกยกเลิก"+' '+'('+$scope.Vouchercancel.length+')';
                }
                if($scope.flagcheck == 3){
                    $scope.Typetext = "已取消"+' '+'('+$scope.Vouchercancel.length+')';
                }
            ...
            if (x==5) {
                if($scope.flagcheck == 1){
                    $scope.Typetext = "Rejected"+' '+'('+$scope.Voucherrej.length+')';
                }
                if($scope.flagcheck == 2){
                    $scope.Typetext = "ถูกปฏิเสธ"+' '+'('+$scope.Voucherrej.length+')';
                }
                if($scope.flagcheck == 3){
                    $scope.Typetext = "已拒绝"+' '+'('+$scope.Voucherrej.length+')';
                }
                $scope.VoucherSelect = '';
   ...
   $scope.dataVCtoderiver = x;
    ...
    $scope.openContact = function(i){
        ...
        var online =  setInterval(function() {
            ...
            .success(function (response) {
                //console.log(response);
                 $scope.timeOp =  parseInt(response.online.time);
                ...
                $http({
                    ...
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                        ...
                        $scope.online = parseInt(response.online.time);
...
$scope.locationall = function(x){
   ...
   $scope.voucherdialogv2 = function(x){
    ...
    $scope.voucherdialog = function(x){
        ...
        $scope.linkvoucher = "http://t-booking.com/demo/print.php?name=admin/voucher&file=transfer&no="+x.TB_order_id+"&order="+x.TB_order_order+"&code="+x.TB_order_code+"";
    ...
    $scope.detailfirsthome =function(x){
        //alert(x)
            ...
            callvoucher($scope.dataVCtoderiver);
...
$scope.takepicturesfun = function(){
       ...
       // var interval3 =  $interval(function() {
        ...
        else if ($scope.imageOne != undefined){
            console.log("!!in case null")
        ...
        console.log(form_data)  
           angular.forEach($scope.imageOne, function(file){ 
                console.log(file) 
               ...
               if (x == 1) {
                    ...
                    $http({
                        ...
                        headers: {'Content-Type': 'application/x-www-form-urlencoded'}
                            ...
                            $scope.anlink = "../demo/load_page.php?name=livechat&file=index&vc="+$scope.sendVCID+"&lang="+$scope.pasa+"&order="+$scope.data+"&chat_from=customer&chat_to=driver";
                              $('#load_data_manage_work').load($scope.anlink);
                              $scope.load = { 'display':'none'};
                              $('#load_data_manage_work').slideToggle();
                            $scope.poupradio = { 'display':'none'};
    ...
    $scope.Eupdatecomment = function(){
        ...
        $scope.language = function(x){
            ...
            if ($scope.numflag==3) {
                if($scope.flagcheck == 1){
                    $scope.Typetext = "Unused";
                }
                if($scope.flagcheck == 2){
                    $scope.Typetext = "รอใช้";
                }
                if($scope.flagcheck == 3){
                    $scope.Typetext = "等待使用";
                }
            ...
            if ($scope.numflag==4) {
                if($scope.flagcheck == 1){
                    $scope.Typetext = "Canceled";
                }
                if($scope.flagcheck == 2){
                    $scope.Typetext = "ที่ถูกยกเลิก";
                }
                if($scope.flagcheck == 3){
                    $scope.Typetext = "已取消";
                }
                //$scope.VoucherSelect = '';
                //$scope.numVocherpass = 0; 

            }
            if ($scope.numflag==5) {
                if($scope.flagcheck == 1){
                    $scope.Typetext = "Rejected";
                }
                if($scope.flagcheck == 2){
                    $scope.Typetext = "ถูกปฏิเสธ";
                }
                if($scope.flagcheck == 3){
                    $scope.Typetext = "已拒绝";
                }
