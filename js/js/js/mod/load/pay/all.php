...
$payot_all= $row['SUM(cost)'];
 ...
 $db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
$query = "SELECT SUM(use_car_overtime_cost_driver_total) FROM  transfer_report_all where transfer_date  like '%" . $no . "%'  and drivername='".$driver_id."' "; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($result)){
$pay_advance_driver= $row['SUM(use_car_overtime_cost_driver_total)'];
 ...
 $db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
$query = "SELECT SUM(use_car_overtime_cost_total) FROM  transfer_report_all where transfer_date  like '%" . $no . "%'  and drivername='".$driver_id."' "; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($result)){
...
///// ฝากเก็บ
 ...
 $db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
$query = "SELECT SUM(cost) FROM  history_transfer_deposit  where transfer_date  like '%" . $no . "%'  and drivername='".$driver_id."' "; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($result)){
...
<table width="100%"  border="0" cellspacing="2" cellpadding="2">
 ...
 <table width="100%"  border="0" cellspacing="2" cellpadding="2">
  <tr  >
	...
	<?= number_format( $arr[web_driver][salary], 0 );?>
...
<? if($arr[web_driver][salary_other]>0){ ?>
<table width="100%"  border="0" cellspacing="2" cellpadding="2">
  <tr  >
    <td  class="font_18" style="height:30px; padding-left:5px; ">เงินประจำตำแหน่ง</td>
    <td width="40%" align="right"  class="font_24" style="padding-right:10px; color:#000000 "> 
	
	<?= number_format( $arr[web_driver][salary_other], 0 );?>
...
<table width="100%"  border="0" cellspacing="2" cellpadding="2">
  <tr  >
	...
	<?= number_format( $arr[web_driver][social_money], 0 );?>
...
<table width="100%"  border="0" cellspacing="2" cellpadding="2">
 ...
 <table width="100%"  border="0" cellpadding="2" cellspacing="2"  style="background-color:#F6F6F6;border:  dotted 1px #999999;  ">
   <tr  >
     ...
     <td width="40%" align="right"  class="font_24" style="padding-right:10px; color: #006699 "><?= number_format( $arr[web_driver][salary]+$arr[web_driver][salary_other]+$payot_all-$arr[web_driver][social_money] -$pay_advance-$pay_deposit, 0 );?>
     </td>
   </tr>
 </table>
