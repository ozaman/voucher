...
font-size:22px;   padding:0px;  color:#666666;  width:100%; background-color:#F6F6F6;      
 border-top: 1px solid #e5e5e5; margin-bottom:  0px; padding-bottom:5px;  
...
<div class="bottom_popup_chat" style="margin:0;  ">   
				  ...
				  <form id="chat_form" name="chat_form"  action="go.php?name=livechat&file=upload_pic&time=<?=time()?>"  method="post" enctype="multipart/form-data"  target="uploadtarget">  
<iframe id="uploadtarget" name="uploadtarget" src="" style="width:200px;height:30px;border:0"></iframe>

<input id="btnupload" type="submit"  class="btn btn-primary"   data-backdrop="false" value="อัพโหลด" style="display:none">

 <div class="box-footer" style="position:fixeds; padding-bottom:5px; margin-top:-20px; margin-left:5px; margin-right:0px; background-color:#FFFFFF">
 ...
 <input type="hidden" name="vc_chat_last_id" id="vc_chat_last_id" value="0"  style="width:50px " /> 
  ...
  <tr>
    <td width="100%" > <input name="message" type="text"  id="message"   style="height:38px; width:100%; margin-right:5px; background-color: " value="" placeholder="เขียนข้อความ ..." onFocus="checktext();"></td>
    <td style="width:35px; padding-left:5px;  "><button type="button" class="btn btn-default" style="margin-right:5px;  margin-left:0px; padding:5px; width:30px; height:38px " id="chat_button_talk"> <i class="fa fa-microphone" style="font-size:26px; color: #333333  "></i>  </button></td>
    <td   style="width:60px;  ">      <button type="button" class="btn btn-default"  data-toggle="dropdown" style="margin-right:0px;  margin-left:0px; padding:2px; width:60px;margin-right:0px; height:38px  " id="chat_button_tool"> <i class="fa fa-plus-square-o" style="font-size:30px; color: #333333 "></i>  </button>
	
	 <button type="button" class="btn btn-primary btn-flat" style="display:none; width:60px; height:38px;"  id="chat_button_submit"><div style="margin-left:0px; ">ส่ง</div></button></td>
  </tr>
</table>
...
<script>
 ...
 $('#load_chat_share_map').load('google/check_system.php');
  ...
  $( "#alert_show_map" ).slideToggle(); 
 $('#load_chat_share_map_realtime').load('google/check_system_realtime.php');
  });
...
///$.post('go.php?name=livechat&file=upload_pic',$('#chat_form').serialize(),function(response){
// $('#send_chat_data').html(response);  });
  ...
  */
  ...
  var url_button = "go.php?name=livechat/sub/load&file=send_button&type=tool";
